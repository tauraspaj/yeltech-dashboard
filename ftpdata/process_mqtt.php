...
$data = file_get_contents('https://mqtt.yelcloud.com/getmqttreadings/100');
...
if ($updatedLastKeyNum > 0) {
    $sql = "
        UPDATE mqttTracker SET lastKeyNum = $updatedLastKeyNum WHERE trackerId=0;
    ";
    $result = mysqli_query($conn, $sql);
}
...
function getLastKeyNum($conn) {
    ...
    $sql = "
    SELECT lastKeyNum FROM mqttTracker WHERE trackerId = 0
    ";
    $result = mysqli_query($conn, $sql);
