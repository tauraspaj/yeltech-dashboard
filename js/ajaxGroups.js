$(document).ready(function () {
			...
			success: function (data) {
				...
				for (i = 0; i < groups.length - 1; i++) {
                    ...
                    <tr class="hover:bg-bluegray-100 h-12 border-b border-gray-200">
                        ...
                        <td class="text-center py-2 px-4 text-sm text-gray-600">`+ groups[i].smsLeft + `</td>
                        <td class="text-center py-2 px-4 text-xs text-gray-600">`+ groups[i].latitude + `<br>`+ groups[i].longitude + `</td>
						...
						<td class="text-center px-6">
							<button id="select" data-id="`+ groups[i].groupId + `" class="focus:outline-none text-xs text-gray-600 uppercase py-1 px-2 bg-gray-50 border border-gray-300 rounded font-medium hover:bg-gray-200">
								Profile
							</button>
						</td>
                    </tr>
	...
	function updateGroupInfo(groupId, groupName, smsLeft, latitude, longitude, dashAccess, appAccess) {
	...
	function showGroupProfile(group) {
		...
		$('#profile-smsLeft').val(group.smsLeft);
	...
	$('#viewgroup-modal').delegate('#saveGroup', 'click', function() {
		...
		updateGroupInfo( groupId, $.trim($('#profile-groupName').val()), $.trim($('#profile-smsLeft').val()), $.trim($('#profile-latitude').val()), $.trim($('#profile-longitude').val()), $.trim($('#profile-dashAccess').val()), $.trim($('#profile-appAccess').val()) ).then( function(response) {
