...
function processBSCmessage($conn, $message) {
    ...
    // EWBs forward all incoming alarms and status messages to recipients
    if ($productName == 'EWB' || $productName == 'EWB V2') {
        if ($message_type == 'ALARM MESSAGE' || $message_type == 'STATUS MESSAGE') {
            forwardEWBalarms($conn, $deviceId, $textBody);
        }
    }
